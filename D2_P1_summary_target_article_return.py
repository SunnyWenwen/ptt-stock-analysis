...
from config import csv_path, xlsx_path, upload_date, return_days_str_list, return_days_map
...
# 前十筆測試用
# ppt_article_for_return = ppt_article_for_return.head(10)
...
all_target_article_return_df = all_target_article_return_df[
    ['author0', 'title', 'date_format', 'url', 'target_code',
     'is_success_get_return_data'] + return_days_str_list].reset_index(drop=True)

# 重新命名欄位
all_target_article_return_df.rename(columns=return_days_map, inplace=True)
